<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="max-width=device-max-width, initial-scale=1.0">
    <title>Simple Bootstrap Template</title>
    <link href="styles.css?v=1" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css"
        integrity="sha512-z3gLpd7yknf1YoNbCzqRKc4qyor8gaKU1qmn+CShxbuBusANI9QpRohGBreCFkKxLhei6S9CQXFEbbKuqLg0DA=="
        crossorigin="anonymous" referrerpolicy="no-referrer" />

</head>

<body>
    <div class="container mt-2">
        <div class="row">
            <div class="col-6">
                <div class="square">
                    <img style="max-height:100%; max-width:100%;" src="ikone/artikalkutija.png" alt="Icon 1">
                    <h6>ARTIKAL - KUTIJA</h6>
                </div>
            </div>
            <div class="col-6">
                <div class="square">
                    <img style="max-height:100%; max-width:100%;" src="ikone/artikalpaleta.png" alt="Icon 2">
                    <h6>ARTIKAL - PALETA</h6>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-6">
                <div class="square">
                    <img style="max-height:100%; max-width:100%;" src="ikone/artikalpaleta.png" alt="Icon 3">
                    <h6>ARTIKAL - REGAL</h6>
                </div>
            </div>
            <div class="col-6">
                <div class="square">
                    <img style="max-height:100%; max-width:100%;" src="ikone/kutijapaleta.png" alt="Icon 4">
                    <h6>KUTIJA - PALETA</h6>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-6">
                <div class="square">
                    <img style="max-height:100%; max-width:100%;" src="ikone/kutijaregal.png" alt="Icon 5">
                    <h6>KUTIJA - REGAL</h6>
                </div>
            </div>
            <div class="col-6">
                <div class="square">
                    <img style="max-height:100%; max-width:100%;" src="ikone/regalregal.png" alt="Icon 6">
                    <h6>REGAL - REGAL</h6>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-6">
                <a href="index.php">
                    <div class="square">
                        <img src="ikone/arrow.png" alt="Icon 3">
                        <h6>NAZAD</h6>
                    </div>
                </a>
            </div>
        </div>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.min.js"></script>
</body>

</html>